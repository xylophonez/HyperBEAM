...
verify(_Base, Req, NodeMsg) ->
    Token =
        hb_ao:get(
            <<"token">>,
            Req,
            hb_opts:get(ao_payment_token, undefined, NodeMsg),
            NodeMsg
        ),
    Ledger =
        hb_ao:get(
    ...
    of
        true ->
            {error, #{
                <<"status">> => 400,
                <<"body">> =>
                    <<"Missing token, ledger, deposit-address, message-id, slot, sender, or quantity.">>
            }};
        false ->
...
fetch_schedule(Token, Slot, NodeMsg) ->
    case mainnet_url(NodeMsg) of
        undefined ->
            {error, missing_mainnet_url};
        BaseURL ->
            SlotBin = hb_util:bin(Slot),
            URL =
                <<BaseURL/binary, "/", Token/binary,
                    "~process@1.0/schedule?from=", SlotBin/binary,
                    "&to=", SlotBin/binary, "&accept=application/aos-2">>,
            fetch_json(URL)
    end.
...
fetch_result(Token, Slot, NodeMsg) ->
    case mainnet_url(NodeMsg) of
        ...
        BaseURL ->
            ...
            URL =
                <<BaseURL/binary, "/", Token/binary,
                    "~process@1.0/compute&slot=", SlotBin/binary,
                    "/results?require-codec=application/json&accept-bundle=true">>,
            fetch_json(URL)
    end.

mainnet_url(NodeMsg) ->
    case hb_opts:get(ao_payment_mainnet_url, undefined, NodeMsg) of
        undefined ->
            undefined;
        URL0 ->
...
fetch_error(missing_mainnet_url) ->
    {error, #{
        <<"status">> => 500,
        <<"body">> => <<"Missing ao-payment-mainnet-url.">>
    }};
fetch_error(Reason) ->
    {error, #{
        <<"status">> => 502,
        ...
        lists:any(
            fun(Edge) ->
                ...
                hb_maps:get(<<"Id">>, Message, undefined, NodeMsg) =:= MessageID
                    andalso tag_value(MessageTags, <<"Action">>, NodeMsg)
                        =:= <<"Transfer">>
                    andalso tag_value(MessageTags, <<"Recipient">>, NodeMsg)
                        =:= DepositAddress
                    andalso hb_util:bin(
                        tag_value(MessageTags, <<"Quantity">>, NodeMsg)
                    ) =:= Quantity
                    andalso hb_util:bin(
                        tag_value(AssignmentTags, <<"Nonce">>, NodeMsg)
                    ) =:= Slot
            end,
            Edges
        ),
    ...
    case {Debit, Credit} of
        ...
        _ ->
            {error, #{
                <<"status">> => 402,
                <<"body">> =>
                    <<"AO result did not contain the expected Debit-Notice and Credit-Notice.">>
            }}
    end.
    ...
    CreditNotice =
        hb_message:commit(
            #{
                ...
                <<"quantity">> =>
                    hb_util:int(
                        hb_maps:get(<<"quantity">>, Payment, undefined, NodeMsg)
                    ),
...
local_node(NodeMsg) ->
    hb_opts:get(
        ao_payment_node,
        <<"http://localhost:",
            (hb_util:bin(hb_opts:get(port, 10000, NodeMsg)))/binary>>,
        NodeMsg
    ).
