...
HyperBEAM transforms the abstract concepts of AO-Core—such as [Messages](/introduction/what-is-ao-core.md#core-concepts), [Devices](/introduction/what-is-ao-core.md#core-concepts), and [Paths](/introduction/what-is-ao-core.md#core-concepts)—into a concrete, operational system. Here's why it's pivotal to the AO ecosystem:

- **Modularity via Devices:** HyperBEAM introduces a uniquely modular architecture centered around [Devices](/introduction/ao-devices.md). These pluggable components define specific computational logic (like running WASM, managing state, or relaying data), allowing for unprecedented flexibility, specialization, and extensibility in a decentralized system.
...
- **Pluggable Devices:** The heart of HyperBEAM's extensibility. It includes essential built-in devices like [`~meta`](/devices/foundations/meta-at-1-0.md), [`~relay`](/devices/foundations/relay-at-1-0.md), [`~process`](/devices/compute-and-processes/process-at-1-0.md), [`~scheduler`](/devices/compute-and-processes/scheduler-at-1-0.md), and [`~wasm64`](/devices/compute-and-processes/wasm-64-at-1-0.md) for core functionality, but the system is designed for easy addition of new custom devices.
...
- **Compute Model:** Computation follows the pattern `Message1(Message2) => Message3`, where messages are resolved through their devices and [paths](/introduction/pathing-in-ao-core.md). The integrity and history of these computations are ensured by **hashpaths**, which serves as a cryptographic audit trail.
- **Scheduler System:** The scheduler component manages execution order using ["slots"](/devices/compute-and-processes/scheduler-at-1-0.md#slot-system) — sequential positions that guarantee deterministic computation.
...
For a comprehensive guide on constructing and interpreting paths in AO-Core, including detailed examples and best practices, see [Pathing in AO-Core](/introduction/pathing-in-ao-core.md).
