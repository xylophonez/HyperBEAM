...
Count and boolean match examples need a first step that writes or imports known data, plus a final step proving the returned match belongs to that data. A node with an empty or different index can otherwise return 500, 0, or a true result for unrelated data.
