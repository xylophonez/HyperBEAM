...
DeviceGroups =
    hb_packager:scan(
        [DeviceSrcDir],
        #{ <<"device-roots">> => DeviceRoots }
    ).

PublishDevice =
    fun(Group) ->
        Pkg = hb_packager:package(Group, LedgerCommitOpts),
        Name = maps:get(device_name, Pkg),
        SpecMsg =
            hb_message:commit(
                hb_packager:spec_message(Pkg, LedgerCommitOpts),
                LedgerCommitOpts
            ),
        ...
        ImplMsg =
            hb_message:commit(
                hb_packager:impl_message(Pkg, SpecID, LedgerCommitOpts),
                LedgerCommitOpts
            ),
        {ok, ImplID} = hb_cache:write(ImplMsg, LedgerCommitOpts),
        #{
            <<"name">> => Name,
            <<"root">> => hb_util:bin(maps:get(root, Group)),
            <<"spec-id">> => SpecID,
            <<"impl-id">> => hb_util:human_id(ImplID),
            <<"module">> => hb_util:bin(maps:get(module_name, Pkg)),
            <<"signer">> => Operator
        }
    end.

PublishedDevices = [PublishDevice(Group) || Group <- DeviceGroups].
DeviceNameResolver =
    maps:from_list([
