...
%% @doc Overwrite each orphaned cached block above the fork with its canonical
%% counterpart before the record is rewound, so the forward resync cannot
%% re-index an orphan. A failed write fails recovery closed.
